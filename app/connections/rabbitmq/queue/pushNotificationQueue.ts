...
const routingQueueKey = process.env.PUSH_TOKEN_NOTIFICATION_QUEUE || 'PUSH_TOKEN_NOTIFICATION_QUEUE';
...
connectionEvent.on('ready', () => {
  ...
  rabbitmq.getRabbitmqInstance().consume(rabbitmq.getChannel(), routingQueueKey, firebaseServiceInstance.pushTokenNotification, consumerOptions)
});
