pipeline {
  ...
  environment {
    BUILD_NAME = "sb11-backend-ms-notification-service"
  ...
  stages {
    stage('SB11 Notification Service Development Build') {
      when {
        beforeInput true
        branch 'development'
      }
      ...
      steps {
        container('docker') {
          withCredentials([[
            ...
            docker build --network=host -t $ECR:$CI_COMMIT_SHA .
    ...
    stage('SB11 Notification Service Staging Build') {
      when {
        beforeInput true
        branch 'staging'
      }
    ...
    stage('SB11 Notification Service Production Build') {
      when {
        beforeInput true
        branch 'production'
      }
